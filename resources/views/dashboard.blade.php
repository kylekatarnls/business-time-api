            ...
            <div class="bg-white overflow-hidden shadow-xl sm:rounded-lg p-6">
                ...
                <form method="POST" action="{{ route('add-authorization') }}">
                    ...
                    <div>
                        ...
                        <x-jet-input id="name" class="block mt-1 w-full" type="text" name="name" :value="old('name')" required :autofocus="!$authorizationsCount && !old('name')" />
                    </div>
                    ...
                    @if(count($authorizations) > 1)
                        <div class="mt-4">
                            ...
                            <div>
                                @foreach($authorizations as $authorizationsData)
                                    <label>
                                        <input type="radio" name="type" value="{{ $authorizationsData->type }}" {!! old('type') === $authorizationsData->type || (($authorizationsData->default ?? false) && ! old('type')) ? 'checked' : '' !!} />
                    ...
                    @foreach($authorizations as $authorizationsData)
                        <div class="mt-4" data-type="{{ $authorizationsData->type }}">
                            ...
                            <x-jet-input id="{{ $authorizationsData->type }}" class="block mt-1 w-full {{ isset($authorisationsErrors[$authorizationsData->type]) ? 'border-red-500' : '' }}" type="text" name="{{ $authorizationsData->type }}" :value="old($authorizationsData->type)" required />
